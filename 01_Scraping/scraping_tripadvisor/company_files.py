...
for nb in range(len(company_list) - 7):
	if nb % 8 == 0:
		if not os.path.exists('scraper/scrap_{}/'.format(nb//8)):
			os.mkdir('scraper/scrap_{}/'.format(nb//8))

		if not os.path.exists('scraper/scrap_{}/company.yaml'.format(nb//8)):
			file = open('scraper/scrap_{}/company.yaml'.format(nb//8),'w')
			config = dict({
